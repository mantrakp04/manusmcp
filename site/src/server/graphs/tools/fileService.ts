...
export const fileReadImageTool = new DynamicStructuredTool({
  ...
  func: async ({ file, content, append, leading_newline, trailing_newline, sudo }) => {
    try {
      ...
      if (sudo) {
        ...
        const tempFile = `/tmp/file_write_${Date.now()}.tmp`;
...
export const fileStrReplaceTool = new DynamicStructuredTool({
  ...
  func: async ({ file, old_str, new_str, sudo }) => {
    try {
      ...
      if (sudo) {
        const tempFile = `/tmp/file_replace_${Date.now()}.tmp`;
...
export const fileFindInContentTool = new DynamicStructuredTool({
...
export const fileFindByNameTool = new DynamicStructuredTool({
